...
def pick_invoice_total(lines, keywords_must_include, ignore_words, regex, lookahead_lines=2):
    ...
    candidates.sort(key=lambda t: (t[2], t[1]))  # prefer later line; tie-break by larger amount
    best = candidates[-1]
...
def merge_date_field(vendor_cfg, global_cfg):
    ...
    return {
        "labels": pick("labels", ["Invoice Date", "Date", "Inv Date", "Billing Date", "Bill Date"]),
        "date_regex": pick(
            "date_regex",
            r"\b(?:\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|[A-Za-z]{3,9}\s+\d{1,2},\s*\d{4}|20\d{2}-\d{2}-\d{2})\b"
        ),
        "ignore_near": pick("ignore_near", ["due", "ship", "rental", "service", "from", "thru", "through", "period"]),
        "header_zone_lines": int(v.get("header_zone_lines", g.get("header_zone_lines", 30))),
        "lookahead_lines": int(v.get("lookahead_lines", g.get("lookahead_lines", 2))),
    }

def _has_date_range(text: str, rx: re.Pattern) -> bool:
    """True if the line looks like a range (contains 2+ dates or words like from/thru/through/to)."""
    low = text.lower()
    if any(w in low for w in (" from ", " thru ", " through ", " to ")):
        return True
    return len(rx.findall(text)) >= 2

def pick_invoice_date(lines, labels, date_regex, ignore_near, header_zone_lines=30, lookahead_lines=2):
    ...
    head = lines[:max(1, header_zone_lines)]

    # 1) HEADER: labeled date (same line), prefer single-date lines
    for ln in head:
        low = ln.lower()
        if any(l in low for l in lbls) and not bad_context(ln):
            if _has_date_range(ln, rx):
                continue
            m = rx.search(ln)
            if m:
                return m.group(0), "header_label_same"

    # 1b) HEADER: labeled date on the next 1-2 lines under the label
    for i, ln in enumerate(head):
        ...
        if any(l in low for l in lbls) and not bad_context(ln):
            for j in range(1, lookahead_lines + 1):
                if i + j < len(lines):
                    nxt = lines[i + j]
                    if bad_context(nxt) or _has_date_range(nxt, rx):
                        continue
                    m2 = rx.search(nxt)
                    if m2:
                        return m2.group(0), "header_label_next"

    # 2) ANYWHERE: labeled date (prefer single-date lines)
    for ln in lines:
        low = ln.lower()
        if any(l in low for l in lbls) and not bad_context(ln):
            if _has_date_range(ln, rx):
            ...
            m = rx.search(ln)
            if m:
                return m.group(0), "anywhere_label"

    # 3) HEADER: stand-alone date not in a bad context and not a range
    for ln in head:
        if bad_context(ln) or _has_date_range(ln, rx):
            continue
        m = rx.search(ln)
        if m:
            return m.group(0), "header_unlabeled"

    # 4) ANYWHERE: stand-alone date not in a bad context and not a range
    for ln in lines:
        if bad_context(ln) or _has_date_range(ln, rx):
            continue
        m = rx.search(ln)
        if m:
            return m.group(0), "anywhere_unlabeled"
...
def process_pdf(pdf_path: Path):
    ...
    # Invoice date with header-first + range skipping
    ...
    inv_date, _ = pick_invoice_date(
        ...
        header_zone_lines=date_field.get("header_zone_lines", 30),
        lookahead_lines=date_field.get("lookahead_lines", 2),
    )
