...
# ---------- invoice total (simpler) ----------
def merge_total_field(vendor_cfg, global_cfg):
    """
    Minimal defaults; vendor YAML can override anchors/regex if needed.
    """
    g = global_cfg["fields"].get("invoice_total", {})
    v = vendor_cfg.get("fields", {}).get("invoice_total", {})
    return {
        "anchors": v.get("anchors", g.get("anchors", [
            "Invoice Amount", "Total Due", "Net Invoice Amount", "Grand Total"
        ])),
        "regex": v.get("regex", g.get("regex",
            r"\$?\s*-?\(?\d{1,3}(?:,\d{3})*(?:\.\d{2})?\)?"
        )),
        # words that often appear with non-final totals; we skip lines containing these
        "discourage": v.get("discourage", g.get("discourage", [
...
def pick_invoice_total(lines, anchors, regex, discourage_words=None, lookahead_lines=2):
    """
    Simple strategy:
      1) If a line contains a known anchor, grab the right-most amount on the same line;
         if none, check the next couple of lines.
      2) Otherwise, look for 'total-ish' lines (without discourage words) and take the largest amount.
      3) As a last resort, pick the largest amount anywhere in the doc.
    """
    rx = re.compile(regex)
    discourage_words = [w.lower() for w in (discourage_words or [])]

    # helper: all currency-looking substrings in a line, keep as text
    def amounts_in(text):
        hits = rx.findall(text)
        # re.findall may return tuples if there are groups; normalize to str
        return [(h if isinstance(h, str) else h[0]).strip() for h in hits]

    # 1) Anchor-first: same line, then a short lookahead
    for i, ln in enumerate(lines):
        low = ln.lower()
        # skip obviously non-final lines if they contain discourage words
        ...
        # choose earliest anchor on the line and search to its right
        a = min(hits, key=lambda x: low.find(x.lower()))
        cut = low.find(a.lower())
        segment = ln[cut + len(a):]
        amts = amounts_in(segment)
        if amts:
            return amts[-1], "total_same_line"

        # try a couple of lines after the anchor (common layout)
        for j in range(1, lookahead_lines + 1):
            if i + j < len(lines):
                ...
                if amts2:
                    return amts2[-1], f"total_next_line_{j}"

    # 2) No anchors found: scan "total-ish" lines, prefer largest numeric value
    totalish = ("total", "amount due", "balance due", "invoice amount", "net invoice amount", "grand total")
    candidates = []
    for ln in lines:
        low = ln.lower()
        if any(t in low for t in totalish) and not any(dw in low for dw in discourage_words):
            amts = amounts_in(ln)
            if amts:
                # pick the right-most amount on that line
                candidates.append(amts[-1])

    if candidates:
        best = max(candidates, key=_amount_to_number)
        return best, "total_totalish_max"

    # 3) Absolute fallback: take the largest amount anywhere
    all_amts = []
    for ln in lines:
        all_amts.extend(amounts_in(ln))
    if all_amts:
        best = max(all_amts, key=_amount_to_number)
        return best, "total_max_in_doc"
